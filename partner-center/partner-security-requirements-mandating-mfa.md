...
2. Jan uruchamia nową sesję przeglądarki i przechodzi do Partner Center przeglądu pulpitu nawigacyjnego (która nie jest chroniona przez usługę MFA). Partner Center przekierowuje Johna do usługi Azure AD w celu zalogowania się.
...
5. Dzień przed zaimplementowaniem usługi MFA dla dowolnego użytkownika przez fabrikam Jan rozpoczyna nową sesję przeglądarki i przechodzi do strony przeglądu pulpitu nawigacyjnego usługi Partner Center (która nie jest chroniona za pomocą usługi MFA). Partner Center przekierowuje Johna do usługi Azure AD w celu zalogowania się bez monitu uwierzytelniania wieloskładnikowego. 
...
W przypadku korzystania z uwierzytelniania aplikacji i użytkowników Partner Center będzie wymagać weryfikacji MFA. Dokładniej mówiąc, gdy aplikacja partnerska chce wysłać żądanie interfejsu API do usługi Partner Center, musi zawierać token dostępu w nagłówku autoryzacji żądania. 
...
Konta partnerów, w tym agenci administracyjni i agenci pomocy technicznej, mogą używać swoich delegowanych uprawnień administratora partnera do zarządzania zasobami klientów za pośrednictwem portali usług Microsoft Online Services, interfejsu wiersza polecenia (CLI) i interfejsów API (przy użyciu uwierzytelniania app+użytkownik).
...
:::image type="content" source="images/MfaRegistration2.png" alt-text="Krok 2 rejestracji usługi MFA.":::
...
##### <a name="issue-4-partner-cannot-implement-mfa-using-ms-authenticator-app"></a>Problem 4. Partner nie może zaimplementować usługi MFA przy użyciu usługi MS Authenticator App
...
- Partner może również zarejestrować się w celu Azure AD — wersja Premium rozwiązań uwierzytelniania wieloskładnikowego innych firm (zgodnych z usługą Azure AD), co może zapewnić dodatkowe metody weryfikacji.
...
Partner ma agentów partnerów, którzy nadal korzystali ze starszych protokołów uwierzytelniania, które nie są zgodne z usługą MFA. Na przykład użytkownicy nadal będą korzystać z programu Outlook 2010, który jest oparty na starszych protokołach uwierzytelniania. Włączenie uwierzytelniania wieloskładnikowego dla tych agentów partnerskich zakłóci korzystanie ze starszych protokołów uwierzytelniania.
...
Przeczytaj wpis na temat uwierzytelniania podstawowego i usługi [Exchange Online,](https://techcommunity.microsoft.com/t5/exchange-team-blog/basic-auth-and-exchange-online-february-2020-update/ba-p/1191282) aby zrozumieć najnowszy plan obsługi starszego [](https://techcommunity.microsoft.com/t5/exchange-team-blog/bg-p/Exchange) uwierzytelniania dla usługi Outlook, i postępuj zgodnie z blogiem zespołu usługi Exchange, aby uzyskać nadchodzące wiadomości. 
...
Partnerzy mogą zgłosić wyjątek techniczny, aby pominąć weryfikację MFA, jeśli napotkają problemy techniczne z usługami Online Services firmy Microsoft i nie istnieje żadne możliwe rozwiązanie ani obejście. Zanim to zrobisz, [przejrzyj listę typowych problemów](#list-of-common-issues) w poprzedniej sekcji.
