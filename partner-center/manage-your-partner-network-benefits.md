---
title: Zarządzanie korzyściami Microsoft Partner Network zarządzania
ms.topic: how-to
ms.date: 10/21/2020
...
>[!IMPORTANT]
>W tym artykule wyjaśniono, jak rozpocząć korzystanie z korzyści Microsoft Partner Network (MPN) w witrynie Partner Center tej witryny. Jeśli nie otrzymano zaproszenia do przeniesienia informacji o firmie z programu Partner Membership Center (PMC) do programu Partner Center, należy nadal zarządzać korzyściami z członkostwa w programie MPN w programie PMC. [Zaloguj się](https://partner.microsoft.com/_login?authType=OpenIdConnect) teraz do pmc.
...
Karta Produkty w chmurze usługi **Azure &** zawiera subskrypcje oparte na użyciu (środki na korzystanie z platformy Azure) i subskrypcje oparte na licencjach (takie jak Office 365), które są dla Ciebie aprowowane, liczbę licencji dla każdego produktu, typ oferty, który aprowizował ten produkt korzyści, typ korzyści (niezależnie od tego, czy pochodzi on z wersji podstawowej, czy przyrostowej) oraz datę wygaśnięcia tej korzyści.
...
1. Wybierz **Visual Studio subskrypcji,** aby wyświetlić listę Visual Studio subskrypcji.
...
4. **Pomoc techniczna (poprawka włamań).** Tego typu pomocy technicznej należy używać, gdy funkcjonalność jest uszkodzona lub gdy coś nie działa zgodnie z oczekiwaniami (na przykład po otrzymaniu komunikatu o błędzie). Oto kilka wskazówek:
   ...
   - Signature Cloud Support: w przypadku partnerów kompetencji Gold/Silver (niedostępnych dla subskrybentów Action Pack) użyj pomocy technicznej produktu **Signature** dla produktów **w chmurze.**
...
2. Aby aktywować dowolną ofertę lub program w ramach wyjść na rynek, musisz najpierw przypisać kontakt marketingowy firmy. Ten kontakt będzie otrzymywać kolejne wiadomości dotyczące aktywnych ofert w zakresie programu Go-To-Market.
