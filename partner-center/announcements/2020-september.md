...
________________
## <a name="partner-center-software-license-key-and-download-fulfillment-notice"></a><a name="17"></a>Partner Center licencji oprogramowania i powiadomienie o realizacji pobierania
...
Program Microsoft CSP zwiększył się wykładniczo, a udział w nim bierze ponad 90 000 firm. Jest to nasz główny ruch sprzedażowy partnera dla małych i średnich firm oraz dla mniejszych klientów firmowych w ramach nowego doświadczenia handlowego. Na podstawie tego tempa ogłosiliśmy dzisiaj, że rozszerzamy możliwości dla partnerów w programie CSP. Szczegółowe informacje znajdują się w [naszym wpisie w blogu partnera](https://blogs.partner.microsoft.com/mpn/expanding-opportunities-for-partners-in-the-cloud-solution-provider-program/).
...
- Partnerzy rozliczani bezpośrednio w programie CSP i dostawcy pośredni będą mieli dostęp do narzędzia przejścia.  
...
- Przejście jest sterowane decyzją klienta i ma miejsce tylko wtedy, gdy obaj partnerzy podjąć jawne działania.  
...
Partnerzy będą mieli możliwość przetestowania tych ulepszeń od października 2020 r. Zdecydowanie zachęcamy do tego, aby skorzystać z tej możliwości, aby lepiej zrozumieć interfejsy API i wprowadzanie danych wymagane do pomyślnej weryfikacji klienta. Wkrótce podamy więcej szczegółowych informacji na temat dokładnego czasu i sposobu uczestnictwa. Istniejące interfejsy PARTNER CENTER API zostaną wycofane przed końcem 2. kwartału roku 201. W tym czasie musisz przejść do korzystania z nowych interfejsów PARTNER CENTER API.

### <a name="testing-update"></a>Aktualizacja testowania 

Daty testów partnerów to od 2 października 2020 r. do 2 grudnia 2020 r. Partnerzy, którzy chcą uczestniczyć w testach, powinni zapoznać się z Partner Center [Education Customer Testing,](https://partner.microsoft.com/resources/detail/partner-center-edu-testing-guide-pdf) aby dowiedzieć się, jak się przygotować, jak się zarejestrować i czego oczekiwać w fazie testowania.

### <a name="library-and-museum-customers"></a>Klienci biblioteki i biblioteki 

Oprócz opisanych wcześniej ulepszeń z przyjemnością ogłaszamy, że w 2. kwartale 2019 r. włączymy oferty o cenie akademickiej dla klientów korzystających z biblioteki i biblioteki, rozszerzając klientów edukacyjnych, za pomocą których można transakcje ofert CSP. 
...
Jeśli masz pytania dotyczące tego powiadomienia, skontaktuj się z [Partner Center pomocą techniczną.](https://partner.microsoft.com/dashboard/support/referrals/servicerequests?category=referrals)
...
Zostaną wprowadzone aktualizacje definicji metryk ujawnionych w interfejsach API użycia usług O365 dla programu CSP, aby pomóc partnerom w mierzeniu użycia W365. 

### <a name="details"></a>Szczegóły

Jest to aktualizacja [powiadomienia](#8) Partner Center 17 września 2020 r. Wprowadzamy aktualizacje, aby upewnić się, że aktualizacje interfejsu API są zgodne z poprzednimi wersjami, aby upewnić się, że użytkownik nie musi wprowadzać żadnych zmian integracji. 
...
Dzięki tej zmianie definicje metryk dostępnych za pośrednictwem następujących interfejsów API użycia licencji CSP zostaną zaktualizowane: 
...
Zaktualizowane definicje są wymienione poniżej. Ponieważ schemat interfejsu API nie ma żadnych zmian, nowa integracja nie będzie wymagana. 

| **Metryka** | **Zaktualizowane definicje** |
|---------|:---------|
|licensesActive |Liczba licencji, które były używane w ciągu ostatnich 28 dni |
|licensesQualified |Liczba licencji wersji próbnej i licencji innych niż próbne dostępne dla wersji SKU produktu |
...
## <a name="upcoming-russian-federal-law-vat-changes"></a><a name="12"></a>Nadchodzące zmiany w podatku VAT w prawie federalnym federacji rosyjskich
...
Zmiany dotyczące podatku VAT wprowadzone przez prawo federalne #265-FZ zostaną wdrożone od 1 stycznia 2021 r.

### <a name="details"></a>Szczegóły

Rosyjski prawo federalne #265-FZ z 31 lipca 2020 r. zmodyfikował podsekw.26 sec.2 art.149 rosyjskich przepisów podatkowych. Zgodnie ze zmianami z dniem 1 stycznia 2021 r. w Rosji przyznanie  wyłącznych praw IP do  oprogramowania i elektronicznych baz danych, które nie są uwzględnione w ujednoliconym rejestrze rosyjskich baz danych oprogramowania i elektronicznych baz danych, a także przyznanie praw do użytkowania takiego oprogramowania i baz danych (w tym aktualizacji i uaktualnień), a także udzielanie dostępu zdalnego do nich za pośrednictwem Internetu, nie będzie już zwalniane z podatku **VAT.** 

**Należy pamiętać, że po powyższych zmianach podatku VAT w stawce określonej przez obowiązujące prawo zostanie dodany przez MIOL do wszystkich nienadmiernych produktów firmy Microsoft w Rosyjskim.**
...
- MBS/Dynamics: Partnerzy powinni rejestrować żądania za pośrednictwem [narzędzia listy OCL](https://businesscenter.mbs.microsoft.com/)w PartnerSource Business Center . 
- CSP: Partnerzy powinni [rejestrować wniosek o pomoc techniczną](https://partnercenter.microsoft.com/partner/support)

### <a name="next-steps"></a>Następne kroki:

Udostępnij te informacje w całej organizacji zgodnie z twoimi potrzebami. Firma Microsoft nie może ci doradzać w sprawie podatku. Należy skonsultować się z własnymi doradcami podatkowymi w sprawie wpływu tej zmiany na Twoją firmę. 
________________

## <a name="partner-center-insights--your-single-reporting-resource-for-understanding-your-business-with-microsoft"></a><a name="10"></a>Partner Center Szczegółowe informacje — pojedynczy zasób raportowania do zrozumienia twojej firmy z firmą Microsoft.
...
- [Raport możliwości klientów i eksport danych:](../insights-customer-opportunities.md)zawiera omówienie możliwości klientów w zakresie zakupu produktów firmy Microsoft.
...
- Zachęcamy do eksplorowania tych nowych możliwości i udostępniania opinii przy użyciu narzędzi do wymiany opinii dostępnych na karcie Szczegółowe informacje danych.
...
Od 22 września 2020 r. partnerzy mogą rozpocząć pobieranie wersji zestawu .NET STANDARD SDK zestawu Microsoft Partner Center .NET SDK w wersji 1.16.2 wraz ze zaktualizowanymi publicznymi zestaw SDK Centrum partnerskiego GitHub **przykładami.**

### <a name="details"></a>Szczegóły

Szczegółowe [informacje Partner Center Microsoft sdk platformy .NET w](https://www.nuget.org/packages/Microsoft.Store.PartnerCenter/1.16.2) wersji 1.16.2 są teraz ogólnie dostępne. Zaktualizowane [GitHub przykłady](https://github.com/Microsoft/Partner-Center-DotNet-Samples) są również dostępne. W tej wersji uwzględniono następujące zmiany:
...
Usługa Azure SQL Edge ma uproszczony cennik odpowiedni dla wdrożeń IoT, dostępny nawet za 60 USD rocznie na urządzenie w ramach zobowiązania 3-letniego lub 10 USD miesięcznie za subskrypcję urządzenia.
...
- Skontaktuj się z odsprzedawcami pośrednimi, którzy nie [](https://partner.microsoft.com/resources/collection/indirect-reseller-onboarding-for-mpa-in-csp#/) zaakceptowali umowy MPA, i podaj im wskazówki krok po kroku w zależności od etapu, na którym znajdują się w Dostawca rozwiązań w chmurze rejestracji programu.
...
#### <a name="offer-details"></a>Szczegóły oferty
   ...
   |Usługi zawartości inteligentnej (non profit staff pricing)|48db24e2-0aac-47bd-bd74-40bde9ff7748|5YI-00005|
...
## <a name="enhanced-incentives-support-experience-and-ticket-creation-process"></a><a name="4"></a>Ulepszone środowisko pomocy technicznej zachęt i proces tworzenia biletu
...
Plan połączeń jest obecnie dostępny w Japonii, Kanadzie, Francji, Niemczech, Azji, Niemczech, Holandia, Portoryko (dostępność kanałów ograniczona tylko do ea i EES), Hiszpania, Zjednoczone Królestwo i Stany Zjednoczone (dostępność planu połączeń innych firm w Australii i Japonii).
