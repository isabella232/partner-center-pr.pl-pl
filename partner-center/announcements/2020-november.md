...
Ogłoszenia dotyczące roku 2020: kwiecień [](2020-april.md)  |  [maj](2020-may.md)  |  [](2020-june.md)  |  [czerwiec lipca](2020-july.md)  |  [](2020-august.md)  |  [sierpień wrzesień](2020-september.md)  |  [](2020-October.md) | Listopada
...
Od 18 listopada Partner Center rozpocznie się nowe środowisko zarządzania kontami, które poprawi środowisko użytkownika dla partnerów. Partnerzy będą teraz mogli zarządzać swoim kontem dla wszystkich programów za > ustawienia konta.
...
Kluczową zmianą w zarządzaniu kontami  jest integracja poprzednio oddzielnego widoku ustawień partnera dla partnerów zarejestrowanych w programach  komercyjnych (Microsoft Partner Network, Dostawca rozwiązań w chmurze, poleceń itp.) i widoku Ustawienia dewelopera dla partnerów zarejestrowanych w programach dla deweloperów (komercyjna platforma handlowa, konsola Xbox w systemie Windows & itp.) w ujednoliconym widoku **ustawień** konta.
...
### <a name="summary"></a>Podsumowanie
Wprowadzamy nową promocję usługi Office 365 E1 dla średniej rynku ograniczoną do nowych rynków LATAM, India i APAC (z wyłączeniem rynków rozwiniętych w ramach APAC).

### <a name="impacted-audience"></a>Odbiorcy, których to miało wpływ
Wszyscy partnerzy w ramach programu Dostawca rozwiązań w chmurze (CSP) na rynkach uczestniczących wymienionych powyżej.

### <a name="details"></a>Szczegóły
Rabat na korzystanie z usługi Office 365 E1 dla programu CSP wynosi 30%. Będzie ona uruchamiana 1 grudnia 2020 r. i będzie dostępna przez 6 miesięcy do 31 maja 2021 r. Ta promocja jest dostępna tylko dla klientów, którzy są nowi w chmurze MW i mają maksymalnie 500 stanowisk. Należy pamiętać, że jest to ograniczone do nowych rynków LATAM, India i APAC, aby umożliwić przejście do chmury. 

#### <a name="offer-details"></a>Szczegóły oferty

   |**Nazwa oferty**|**Identyfikator oferty**|
   |-------------------|:------|
   |Office 365 E1 dla średnich promocji|d20fdad5-f524-45e4-97b1-59a94c21778f|
...
- Partner współpracuje z nowymi lub istniejącymi klientami w następujących krajach: Nawiązano współpracę z następującymi krajami: Armenią (AM), Kolumbią (AZ), Kolumbią (HU), Kirgizstanem (KG), Kolumbią (RU), Tadżykistanem (TJ), Stanem (UZ) i Stanami (UA) 

W przypadku takich partnerów z klientem w tych krajach partner będzie teraz miał możliwość przesyłania identyfikatora prawnego określonego klienta **(znanego** także jako organizacja **KLIENTA** klienta) i numeru telefonu klienta, gdy partner spróbuje zaktualizować lub utworzyć subskrypcję dla tego klienta.  Ci partnerzy mogą teraz również wprowadzić opcjonalne, środkowe **imię** klienta.

### <a name="partner-center-enhancements"></a>Partner Center ulepszenia

- Partner Center interfejsu użytkownika w celu poprawy weryfikacji klientów w określonych krajach

- Nowe interfejsy API do obsługi ulepszonej weryfikacji klientów w określonych krajach

**Zmiany w interfejsie użytkownika Partner Center — konto klienta**

Strona Partner Center konta klienta została zaktualizowana w następujący sposób:

1. **Organizacja JEDNOSTKI** (znana także jako identyfikator prawny). To pole jest teraz wymagane dla klientów w określonych krajach. 

   > [!NOTE]
   > Dane muszą być wprowadzane tylko w postaci znaków alfanumerycznych, bez łączników, znaków specjalnych ani spacji. Partnerzy będą musieli skontaktować się ze swoimi kontaktami z klientami w tych krajach, aby potwierdzić stan identyfikatora prawnego klienta, taki jak numer PODATKOWY lub NUMER IDENTYFIKACYJNY klienta. Aby uzyskać więcej informacji specyficznych dla kraju, zobacz następującą tabelę.  

2. **Drugie imię.** To pole jest opcjonalne dla wszystkich klientów. Jeśli znasz drugie imię lub inicjał klienta, partner może je wprowadzić tutaj.

3. **Numer telefonu.** To pole było wcześniej opcjonalne, ale jest teraz wymagane dla klientów w określonych krajach.  

Poniżej przedstawiono wymagania dotyczące pól **ORGANIZACJI (identyfikator** prawny) dla poszczególnych krajów.

| Kraj | A14-Area | Kod kraju ISO | Wymagania dotyczące znaków pola | Nazwa pola specyficznego dla kraju w języku natywnym | Tłumaczenie lub komentarze w języku angielskim |
|---------|----------|------------------|------------------------|--------------------|--------------------------------------|
| Armenia | Cee      | AM               | 8 lub 10 cyfr         | :::no-loc text="УНН/NZOU/НЗОУ/ հանրային ծառայության համարանիշը"::: | UNN/numer usługi publicznej                     |
| Azerbejdżan| Cee      | AZ               | 7 lub 10 cyfr         | :::no-loc text="ИНН/VOEN/ПИК"::: | TIN/ICH/PIC              |
| Białoruś | Cee      | BY               | 9 cyfr         | :::no-loc text="УНП"::: | UNP (oznacza to identyfikator podatkowy)                    |
| Węgry | Cee      | HU               | 10 lub 11 cyfr         | :::no-loc text="TIN"::: | NIP (nip)                     |
| Kazachstan | Cee      | KZ              | 12 cyfr         | :::no-loc text="БИН/ИИН"::: | BIN/IIN                     |
| Kirgistan | Cee      | KG               | 14 cyfr         | :::no-loc text="ИНН"::: | UNN/numer usługi publicznej                     |
| Mołdawia  | Cee        | MD               | 6, 7, 8 lub 13 cyfr         | :::no-loc text="IDNO/IDNP"::: | Unikatowy numer identyfikacyjny stanu (IDNO) przypisany do jednostki prawnej jest również dzielony jako kod fiskalny. \"Rejestr stanowy jednostek prawnych dotyczący firm zarejestrowanych w Republice Ludowej opublikowany przez Urząd ds. technologii informatycznych i komunikacyjnych w portalu rządowym programu open data data.gov.md.\"                    |
| Rosja | Cee      | RU               | 9, 10 lub 12 cyfr         | :::no-loc text="ИНН"::: | Inn                     |
| Tadżykistan | Cee      | Tj.               | 9, 10 lub 12 cyfr     | :::no-loc text="ИНН/ЕИН/КПП"::: | NIEAKCYJNA/EIN/KPP                     |
| Uzbekistan | Cee     | UZ               | 9 cyfr                 | :::no-loc text="INN":::         | Inn                               |
| Ukraina   | Cee      | UA               | 8, 10 lub 12 cyfr      | :::no-loc text="EGRPOU/EDRPOU":::        | Kod identyfikacyjny jednostki prawnej                             | 

**Zmiany interfejsu API Partner Center API**
...
Firma Microsoft implementuje ograniczanie przepustowości interfejsu API w I kwartale 2021 r., aby zapewnić bardziej spójną wydajność w zakresie czasu dla partnerów wywołujących interfejsy API Partner Center API. Ograniczanie ogranicza liczbę żądań do usługi w określonym okresie, aby zapobiec nadużytości zasobów. Po przekroczeniu progu ograniczania Partner Center ograniczy wszelkie dalsze żądania od tego klienta przez określony czas.  
...
|{baseURL}/v1/customers/{customer-tenant-id}/transfers|[Uzyskiwanie wszystkich transferów klientów](/partner-center/develop/get-subscription-registration-status)| 
...
- W lutym tego roku poproszono partnerów o podpisanie Umowa z Klientem Microsoft (MCuA). Była to migracja z poprzedniej Umowa dotycząca platformy Microsoft Cloud (MCA). 
...
- Nie wszyscy partnerzy uwzględniono żądanie parametru w ramach ich implementacji. Firma Microsoft zwróciła umowa MCA do tych partnerów.
- Partner będzie następnie ponownie wysłać żądanie podpisania do klienta i ponownie wysłać umowę MCA do firmy Microsoft. 
- Duplikowanie miało wpływ na możliwość świadczenia usług partnerom przez firmę Microsoft.
- We wrześniu 2020 r. wysłaliśmy powiadomienie do partnerów za pośrednictwem usługi Yammer na wielu forach z prośbą o naprawienie parametru przez partnerów. Firma Microsoft nie mogła już zaakceptować duplikatów i otrzyma błędy 409.
...
- Jednak 14 stycznia 2021 r. ponownie nie będziemy akceptować duplikatów. Dzięki temu partnerzy mogą mieć więcej czasu na dostosowanie swoich możliwości. Otrzymaliśmy już powiadomienie od partnera, że planuje on wdrożyć aktualizację 11/16, nad którą będziemy ściśle współpracować.
...
Testowanie jest teraz dostępne dla Partner Center interfejsu API i ulepszeń interfejsu użytkownika dla procesu weryfikacji klientów w edukacji.
...
Aby lepiej zrozumieć interfejsy API i wprowadzanie danych wymaganych do pomyślnej weryfikacji klienta, partnerzy będą mogli przetestować te ulepszenia od października 2020 r. Wkrótce podamy więcej szczegółowych informacji na temat harmonogramu i sposobu uczestnictwa. Istniejące interfejsy PARTNER CENTER API zostaną wycofane przed końcem 2. kwartału roku 2019. W tym czasie musisz przejść do nowych interfejsów API Partner Center api.

   - Testowanie dostępne Daty testu partnera to od 2 października do 2 grudnia 2020 r. Partnerzy, którzy chcą wziąć udział w testach, powinni pobrać Partner Center Education customer testing guide (Podręcznik testowania klientów w edukacji) dotyczący tego, jak przygotować się, jak się zarejestrować i czego oczekiwać w fazie testowania.
...
- Zarejestruj się, aby wziąć udział w testowaniu. (Aby uzyskać szczegółowe [informacje, zobacz Przewodnik](https://partner.microsoft.com/resources/detail/partner-center-edu-testing-guide-pdf) testowania). 
...
## <a name="microsoft-teams-rooms-premium-sku-launch"></a><a name="5"></a>Uruchamianie wersji SKU w witrynie Microsoft Teams Rooms Premium
...
Dostępna jest teraz wersja Premium SKU pomieszczeń microsoft Teams dla usługi Microsoft Teams Dostawca rozwiązań w chmurze programie Dostawca rozwiązań w chmurze (CSP).
...
Nowa wersja SKU Microsoft Teams Rooms Premium dla pomieszczeń Microsoft Teams jest teraz dostępna dla klientów, którzy kupują program CSP za pośrednictwem usługi w wysokości 50 USD za urządzenie miesięcznie. SKU Microsoft Teams Rooms Premium jest alternatywą dla standardowej wersji SKU (wcześniej nazywanej Sala konferencyjna licencji). Ta sku zawiera wszystko w ofercie standardowej, takie jak niezbędne licencje dla aplikacji Microsoft Teams, usługi Skype dla firm Online i zarządzania w usłudze Intune. Ta oferta umożliwia również korzystanie z systemu telefonicznego — niezbędnego do integracji z siecią PSTN (Public Switched Telephone Network) — i wideokonferencji, jeśli jest dostępna. 

Dzięki ofercie Premium klienci mają dostęp do nowo wydanych usług Microsoft Teams Rooms Managed Services, w których eksperci obsługują zarządzanie salami konferencyjnymi i operacje w ich imieniu. Ta oparta na chmurze usługa monitorowania i zarządzania IT zapewnia, że urządzenia w pomieszczeń microsoft Teams i ich urządzenia peryferyjne są aktualne i bezpieczne. Są one również aktywnie monitorowane i zarządzane, utrzymując środowisko zoptymalizowane pod kątem doskonałego środowiska w pomieszczeniu.

#### <a name="released-at-launch"></a>Wydane podczas uruchamiania

   |****|**Urządzenie Microsoft Teams Rooms Standard w USD15 miesięcznie**|**Urządzenie Microsoft Teams Rooms Premium USD50 miesięcznie**|
   |-------------------|:------|:------|
   |Skype dla firm|Tak| |
   |Microsoft Teams|Tak|Tak|
   |Phone System|Tak|Tak|
   |Audiokonferencje|Tak|Tak|
   |Microsoft Intune|Tak|Tak|
   | |Microsoft Teams Rooms Standard 15 USD za urządzenie na miesiąc|Microsoft Teams Rooms Premium USD50 za urządzenie miesięcznie|
   |Microsoft Teams Rooms Managed Services| |Tak|
...
- Proaktywne zarządzanie: 24 godziny na godziny, 7 dni w roku, 365 dni w roku, zarządzanie systemami pomieszczeniami, w tym stosowanie poprawek, zarządzanie konfiguracją i nie tylko.
...
- Aktualizacje zarządzane: zarządzanie i dostarczanie aktualizacji aplikacji, aktualizacji bazy wiedzy systemu Windows i oprogramowania układowego.
...
- Pomoc techniczna dla klientów: 24 godziny na godziny w godzinach 7x365 w naszym dedykowanym centrum operacji obsługi z asystowaną obsługą zdarzeń w przypadku przypadków nie zautomatyzowanych. Szczegółowa kontrola dostępu oparta na rolach.
- Szczegółowe informacje i rekomendacje: zagregowane szczegółowe informacje dla klientów i raporty dotyczące kondycji pomieszczenia, spisu, użycia, spotkań online i trendów zdarzeń.
...
- Dowiedz się więcej o pomieszczeniach firmy Microsoft i powiązanych ofertach na stronie [Microsoft Team Rooms](https://rooms.microsoft.com/).
- Skorzystaj z [przewodnika dla partnerów usługi Teams,](https://aka.ms/teamscallingmeetingsguide) aby opracować swoją praktykę z użyciem sal konferencyjnych usługi Teams i utworzyć gotową ofertę do współpracy.
- Zapoznaj się z [często zadawanymi pytaniami na](https://aka.ms/PartnerMTRFAQ) temat pomieszczeń usługi Microsoft Teams, aby dowiedzieć się więcej o rozwiązaniu i usługach produktu. 
...
## <a name="new-microsoft-teams-advanced-communications-skus-for-government-community-cloud-gcc-coming-soon"></a><a name="4"></a>Nowe jednostki SKU komunikacji zaawansowanej usługi Microsoft Teams Government Community Cloud (GCC) już wkrótce
...
Nowa wersja dodatku Advanced Communications dla usługi Microsoft Teams w witrynie GCC będzie dostępna od 1 grudnia 2020 r.
...
Nowy dodatek Advanced Communications dla usługi Microsoft Teams w GCC jest teraz dostępny w wysokości 12 USD za użytkownika miesięcznie. Dodatkowe jednostki SKU można kupić na podstawie dowolnego innego pakietu Microsoft 365, który zawiera usługę Microsoft Teams. Zaawansowana komunikacja zapewnia nowy zestaw funkcji na duże spotkania, zasady komunikacji, integracje i zaawansowane narzędzia do zarządzania IT. 

#### <a name="offer-details"></a>Szczegóły oferty

   |**Nazwa oferty**|**Identyfikator oferty**|**Identyfikator materiału**|
   |-------------------|:------|:------|
   |Advanced Communications dla GCC|56fe76f5-f4ba-4fac-9561-d0daf59b01a1|7FB-00003|

#### <a name="frequently-asked-questions"></a>Często zadawane pytania 

**Co to jest advanced communications?** Ten nowy dodatek microsoft Teams umożliwia klientom dalsze ulepszanie ich obsługi komunikacji. Można go kupić na podstawie dowolnej subskrypcji Microsoft 365, która jest subskrybowana.

**Ile to kosztuje?** Komercyjna system ERP to 12 USD na użytkownika miesięcznie.

**Którzy klienci mogą kupić dodatek?** Klienci korzystający z GCC mogą zakupić dodatek.
...
**Gdzie można je sprzedać?** Można go sprzedać na rynkach w USA.

**Jakie są wymagania wstępne?** Ten Microsoft 365 może mieć dowolny pakiet usługi Office 365, który zawiera usługę Microsoft Teams.

### <a name="next-steps"></a>Następne kroki

Udostępnij te informacje odpowiednim kontaktom w twojej organizacji i poznaj możliwości sprzedaży krzyżowej i sprzedaży. Zapoznaj się z sekcją zasobów przewodnika [dla partnerów usługi Teams.](https://aka.ms/teamscallingmeetingsguide)
...
1 listopada 2020 r. firma Microsoft opublikowała oferty operacji projektu usługi Dynamics 365 i usunęła usługę Dynamics 365 Project Service Automation (PSA) dla klientów komercyjnych. Ten komunikat zawiera dodatkowe informacje dotyczące mapowania praw podwójnego użycia z tego uruchomienia oraz nowych ofert osadzonych niezależnych dostawców oprogramowania.

#### <a name="project-operations-isv-embed-offers"></a>Oferty osadzania dla isv operacji projektu

1 listopada 2020 r. firma Microsoft opublikowała trzy dodatkowe 36-miesięczne oferty osadzania isv dla usługi Dynamics 365 for Project Operations dla klientów korzystających Dostawca rozwiązań w chmurze (CSP). Zobacz kartę Project Operations (Operacje projektu) w dokumencie Dynamics CSP Offers-November 2020 Excel (Program Excel dla programu [Dynamics CSP 2020),](https://partner.microsoft.com/resources/detail/dynamics-csp-offers-november-xls) aby uzyskać szczegółowe informacje o sku.

#### <a name="project-operations-dual-use-rights-mapping"></a>Mapowanie praw do operacji projektu do podwójnego użycia
...
Informacje na temat mapowania lokalnych praw podwójnego użycia usługi Dynamics 365 dla ofert Project Operations można znaleźć w poniższej tabeli:
   ...
   |**Oferta operacji D365**|**Mapowanie lokalnych praw podwójnego użycia usługi D365**|
   |-------------------|:------|
   |Oferta operacji D365|Mapowanie lokalnych praw podwójnego użycia usługi D365|
   |Operacje projektu D365|D365 for Operations, on-prem (AX Server) using the Dyn365 Project Operations SKU (109108477)|
   |Dołączanie operacji projektu D365|D365 for Operations, on-prem (AX Server) using the Dyn365 Project Operations SKU (109108477)|
   |D365 Finance with Project Operations|D365 for Operations, on-prem (AX Server) using the Dyn365 Project Operations SKU (109108477)|
   |D365 Finance Attach with Project Operations|D365 for Operations, on-prem (AX Server) using the Dyn365 Project Operations SKU (109108477)|
   |Ujednolicone operacje D365 — działanie w operacjach projektu|D365 for Operations, on-prem (AX Server) using the Dyn365 Project Operations SKU (109108477)|

#### <a name="previously-announced"></a>Wcześniej ogłoszono

1 listopada 2020 r. firma Microsoft opublikowała następujące nowe i zaktualizowane usługi Dynamics 365 i Power Platform i produktów dla programu CSP:

- Dynamics 365 Customer Voice USL 

Przejrzyj kartę Project Operations (Operacje projektu) w dokumencie Dynamics CSP Offers-November 2020 Excel (Program Excel dla programu [Dynamics CSP 2020),](https://partner.microsoft.com/resources/detail/dynamics-csp-offers-november-2020-xls) aby uzyskać szczegółowe informacje o identyfikatorze oferty.
...
1 listopada 2020 r. firma Microsoft wprowadziła nowe jednostki SKU dla głosu biznesowego. Pełny pakiet jest dostępny w Kanadzie, Zjednoczonym Królestwie i Stany Zjednoczone. Funkcja Business Voice bez planu połączeń będzie dostępna na wszystkich innych rynkach. 

Microsoft 365 Business Voice to oparty na chmurze system telefonów dla małych i średnich firm, który jest wbudowany w usługę Office 365. Dodanie usługi Business Voice do subskrypcji usługi Office 365 klienta zapewnia rozwiązanie do komunikacji i współpracy typu "wszystko w jednym" z połączeniami, czatami i spotkaniami w jednej aplikacji, Microsoft Teams.
...
## <a name="cloud-solution-provider-csp-promo-for-microsoft-365-business-voice-is-now-available"></a><a name="1"></a>Dostawca rozwiązań w chmurze (CSP) dla usługi Microsoft 365 Business Voice jest teraz dostępna
...
Cennik rabatu dostępny dla nowych i odnowień subskrypcji usługi Microsoft 365 Business Voice z planem połączeń i usługą Microsoft 365 Business Voice bez planu połączeń.
...
Od 1 listopada 2020 r. do 30 kwietnia 2021 r. nowe i odnowienia subskrypcji usługi Microsoft 365 Business Voice z planem połączeń i usługą Microsoft 365 Business Voice bez planu połączeń podlegają cennikowi rabatu. Microsoft 365 Business Voice z planem połączeń jest objęte rabatem 25% przez 12 miesięcy, a u usługi Microsoft 365 Business Voice bez planu połączeń 33% przez 12 miesięcy. 

#### <a name="offer-details"></a>Szczegóły oferty

   |**Nazwa oferty**|**Identyfikator oferty**|**Identyfikator materiału**|
   |-------------------|:------|:------|
   |Microsoft 365 Business Voice Adoption (Rabat za wdrożenie usługi Microsoft 365 Business Voice)|e7d1d0fa-b769-45c7-aaea-c3e6f7402691|PZX-00006|
   |Microsoft 365 Business Voice Adoption (Rabat za wdrożenie usługi Microsoft 365 Business Voice)|ef3ff6bb-a288-4a56-9204-97b37ff9a0b8|PZW-00019|
   |Microsoft 365 Business Voice (US) Adoption (Rabat za wdrożenie usługi Microsoft 365 Business Voice (US)|4244aed3-90ae-4754-8dc8-37f2e8d84e85|PZW-00020|
   |Microsoft 365 Business Voice (bez planu wywołań) ( Promocja wdrożenia)|b71df433-6fd9-4549-886d-577f7aaa06070|PZY-00019|
   |Microsoft 365 Business Voice (bez planu wywołań) ( Promocja wdrożenia)|4ba4d580-4902-42b0-8411-a27358dd5405|PZY-00016|
   |Microsoft 365 Business Voice (bez planu wywołań) ( Promocja wdrożenia)|bbfd896b-e3d4-45ba-9319-14104d400069|PZY-00018|
...
- Odwiedź stronę [Microsoft 365 Business Voice,](https://www.microsoft.com/microsoft-365/partners/businessvoice) aby dowiedzieć się więcej na temat usługi Business Voice. 
...
- Zwiększenie świadomości wśród klientów na temat wartości dodawania głosu biznesowego do usługi Teams. 
