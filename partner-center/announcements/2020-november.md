...
### <a name="next-steps"></a>Następne kroki
Ta informacja będzie dostępna 1 grudnia — w międzyczasie udostępnij te informacje wszystkim odpowiednim kontaktom w organizacji (w regionach uczestniczących). Rozpocznij angażowanie klientów, którzy nie znajdują się jeszcze w chmurze w uczestniczących regionach. 
...
W przypadku takich partnerów z klientem w tych krajach partner będzie teraz musi przesłać identyfikator prawny określonego klienta **(nazywany** również identyfikatorem **PRAWNA** organizacji klienta) i numer **Telefon** klienta, gdy partner spróbuje zaktualizować lub utworzyć subskrypcję dla tego klienta. Ci partnerzy mogą teraz również wprowadzić opcjonalne, środkowe **imię** klienta.
...
**Zmiany w interfejsie Partner Center użytkownika — strona Konto klienta**
...
3. **Telefon Numer.** To pole było wcześniej opcjonalne, ale jest teraz wymagane dla klientów w określonych krajach.  

Wymagania dotyczące pola **ORGANIZATION (Legal** Identifier) organizacji specyficzne dla kraju są widoczne poniżej.
...
|---------|----------|------------------|------------------------|--------------------|--------------------------------------|
| Armenia | Cee      | AM               | 8 lub 10 cyfr         | :::no-loc text="УНН/NZOU/НЗОУ/ հանրային ծառայության համարանիշը"::: | NUMER UNN/numer usługi publicznej                     |
...
| Węgry | Cee      | HU               | 10 lub 11 cyfr         | :::no-loc text="TIN"::: | TIN (nip)                     |
| Kazachstan | Cee      | KZ              | 12 cyfr         | :::no-loc text="БИН/ИИН"::: | BIN/IIN                     |
| Kirgistan | Cee      | KG               | 14 cyfr         | :::no-loc text="ИНН"::: | NUMER UNN/numer usługi publicznej                     |
| Mołdawia  | Cee        | MD               | 6, 7, 8 lub 13 cyfr         | :::no-loc text="IDNO/IDNP"::: | Unikatowy numer identyfikacyjny stanu (IDNO) przypisany do jednostki prawnej jest również dzielony jako kod obrachunkowy. \"Rejestr stanowy jednostek prawnych dotyczący firm zarejestrowanych w Republice Ludowej publikowany przez Rządową Komisja Ds. Technologii Informatycznych i Komunikacyjnych w portalu rządowym programu "open data data.gov.md".\"                    |
...
**Zmiany w interfejsie API Partner Center API**
...
## <a name="euefta-change-of-partner-billing-currency-for-new-commerce-offers"></a><a name="10"></a>Zmiana waluty rozliczeniowej partnera dla nowych ofert handlowych w Unii Europejskiej/UNII EUROPEJSKIEJ.  
...
W regionie Unii Europejskiej (UE)/European Free Trade Association (ASSOCIATION) wszystkie nowe oferty handlowe w programie Dostawca rozwiązań w chmurze będą korzystać z lokalizacji rozliczeniowej partnera, a nie lokalizacji rozliczeń klienta. Oznacza to, że partnerzy będą rozliczani przez firmę Microsoft na podstawie ich waluty lokalizacji, a nie waluty lokalizacji ich klientów. Zostanie to zrobione w dwóch fazach: 
...
>[Uwaga] Ta zmiana będzie mieć wpływ tylko na walutę rozliczeniową partnera, a nie na ceny nowych ofert handlowych w programie CSP. 
...
- Ta aktualizacja zmniejszy złożoność i obciążenie związane z fakturowaniem wielowalutowym w regionie UNII EUROPEJSKIEJ/COMMERCE na nowe środowisko handlowe.  
...
Przejrzyj informacje dotyczące tego tematu w [Galerii operacji]( https://partner.microsoft.com/resources/collection/eu-efta-changes-collection#/ w witrynie internetowej partnerów firmy Microsoft.  
...
Firma Microsoft wprowadza ograniczanie przepustowości interfejsów API dla partnerów wywołujących interfejsy API Partner Center w celu zapewnienia bardziej spójnej wydajności w danym okresie.
...
Firma Microsoft implementuje ograniczanie przepustowości interfejsu API w I kwartale 2021 r., aby zapewnić bardziej spójną wydajność w okresie dla partnerów wywołujących interfejsy API Partner Center API. Ograniczanie ogranicza liczbę żądań do usługi w określonym czasie, aby zapobiec wywłaszceniu zasobów. Po przekroczeniu progu ograniczania Partner Center ograniczy wszelkie dalsze żądania od tego klienta przez określony czas.  
...
|{baseURL}/v1/customers/{identyfikator-dzierżawy-klienta}/transfers|[Pobierz wszystkie przeniesienia klienta](/partner-center/develop/get-subscription-registration-status)| 
...
To zawiadomienie ma na celu zapewnienie partnerom wczesnej świadomości przyszłych zmian, dzięki czemu mogą się przygotować. Zdecydowanie zalecamy, aby partnerzy zaznajomili się z tymi interfejsami API i rozważyli użycie interfejsu API dziennika aktywności w celu zwiększenia wydajności i uniknięcia ograniczania. Aby uzyskać więcej informacji na temat tej funkcji, zapoznaj się ze szczegółami w przewodniku [API throttling guidance (Wskazówki dotyczące ograniczania interfejsu API).](/partner-center/develop/api-throttling-guidance) 

### <a name="next-steps"></a>Następne kroki

Przejrzyj [wskazówki dotyczące ograniczania przepustowości interfejsu API](/partner-center/develop/api-throttling-guidance)i podjąć niezbędne kroki. 
...
- W lutym tego roku partnerzy zostali poproszoni o podpisanie Umowa z Klientem Microsoft (MCuA). Była to migracja z poprzedniej Umowa dotycząca platformy Microsoft Cloud (MCA). 
- W ramach tej zmiany partnerom zażądano, aby uwzględnili parametr typu umowy, zgodnie z [dokumentacją w tym miejscu.](/partner-center/develop/get-confirmation-of-customer-agreement)

### <a name="what-happened-next"></a>Co się stało dalej:

- Nie wszyscy partnerzy uwzględniali żądanie parametru w swojej implementacji. Firma Microsoft zwróciła umowa MCA tym partnerom.
- Partner może następnie ponownie wysłać żądanie podpisania do klienta i ponownie wysłać umowę MCA do firmy Microsoft. 
- Duplikowanie miało wpływ na zdolność firmy Microsoft do świadczenia usług partnerom.
- We wrześniu 2020 r. wysłaliśmy powiadomienie do partnerów za pośrednictwem Yammer na wielu forach z prośbą o naprawienie parametru przez partnerów. Firma Microsoft nie może już akceptować duplikatów i będą otrzymywać błędy 409.
...
- Obecnie przypominamy partnerom, a także wysyłamy osobiste wiadomości e-mail do 10 najpotrzebnszych osób odmówień, aby przejrzeli ich żądania i spisali się do nas, abyśmy pomogli im przetestować i rozwiązać problem.
...
- Jednak 14 stycznia 2021 r. ponownie nie będziemy akceptować duplikatów. Dzięki temu partnerzy będą mieć więcej czasu na korektę po swojej stronie. Otrzymaliśmy już powiadomienie od partnera, że planuje on wdrożyć aktualizację 11/16, nad którą będziemy ściśle współpracować.
- Prosimy partnerów o s skontaktować się z nami, abyśmy pomogli im w testach, dodając dzierżawy do lotu ze zmianami, aby zapewnić, że ich aktualizacja rozwiązania działa zgodnie z oczekiwaniami.


### <a name="next-steps-for-partners"></a>Następne kroki dla partnerów

- Przejrzyj te nowe zmiany interfejsu Partner Center interfejsu użytkownika i interfejsu API. Przy następnej próbie utworzenia subskrypcji klienta lub zaktualizowania istniejącej subskrypcji dla klienta w jednym z tych krajów sprawdź informacje o koncie klienta, aby upewnić się, że dodano dane do dwóch nowych, wymaganych pól dla tego klienta: Organization JEDNOSTKI (identyfikator prawny) i numer Telefon. Te informacje można zaktualizować za pomocą pulpitu Partner Center [nawigacyjnego](https://partner.microsoft.com/dashboard) lub Partner Center [API.](/partner-center/develop/create-a-customer#company-profile)

- Udostępnij te informacje odpowiednim zespołom w organizacji, aby ułatwić im przygotowanie się do tych zmian.

______________

## <a name="testing-available-partner-center-api-and-user-interface-ui-enhancements-for-the-education-customer-validation-process"></a><a name="7"></a>Dostępne testy: Partner Center interfejsu API i interfejsu użytkownika na potrzeby procesu weryfikacji klientów w edukacji
...
Testowanie jest teraz dostępne dla aktualizacji Partner Center API i ulepszeń interfejsu użytkownika dla procesu weryfikacji klientów w edukacji.
...
Aby lepiej zrozumieć interfejsy API i wprowadzanie danych wymaganych do pomyślnej weryfikacji klienta, partnerzy będą mogli przetestować te ulepszenia od października 2020 r. Wkrótce podamy więcej szczegółowych informacji na temat dokładnego czasu i sposobu uczestnictwa. Istniejące interfejsy PARTNER CENTER API zostaną wycofane przed końcem 2. kwartału roku 201. W tym czasie musisz przejść do nowych interfejsów API Partner Center API.
   ...
   - Testowanie dostępne Daty testów partnerów to od 2 października do 2 grudnia 2020 r. Partnerzy, którzy chcą wziąć udział w testach, powinni pobrać Partner Center Education customer testing guide for how to get ready, how to sign up and what to expect during the testing phase ( Partnerzy, którzy chcą uczestniczyć w testach, powinni pobrać Partner Center Education Customer Testing Guide, aby dowiedzieć się, jak się przygotować, jak się zarejestrować i czego oczekiwać w fazie testowania.

**Klienci biblioteki i biblioteki**

Oprócz tych ulepszeń z przyjemnością ogłaszamy, że w 2. kwartale 2019 r. włączymy oferty o cenach akademickich dla klientów korzystających z biblioteki i biblioteki oraz firmy Ich firmy, rozszerzając klientów edukacyjnych, u których można transacyjnie korzystać z ofert CSP.
...
## <a name="expanding-commercial-licensing-partner-content-on-the-operations-readiness-gallery"></a><a name="6"></a>Rozszerzanie zawartości partnera licencjonowania komercyjnego w galerii gotowości operacji
...
## <a name="microsoft-teams-rooms-premium-sku-launch"></a><a name="5"></a>Microsoft Teams Rooms Premium uruchomieniu SKU
...
Microsoft Teams Rooms Premium SKU dla Microsoft Teams za pośrednictwem programu Dostawca rozwiązań w chmurze (CSP) jest teraz dostępna.
...
Nowa Microsoft Teams Rooms Premium SKU dla Microsoft Teams Rooms jest teraz dostępna dla klientów, którzy kupują za pośrednictwem Ciebie program CSP po 50 USD za urządzenie miesięcznie. Ta Microsoft Teams Rooms Premium SKU jest alternatywą dla standardowej Microsoft Teams Rooms (wcześniej nazywanej Sala konferencyjna Licencji). Ta usługa SKU obejmuje wszystkie elementy oferty standardowej, takie jak niezbędne licencje na usługi Microsoft Teams, Skype dla firm Online i zarządzanie usługą Intune. Ta oferta umożliwia również system telefoniczny — niezbędne do integracji publicznej sieci pstn — i audiokonferencje, jeśli są dostępne. 
...
#### <a name="released-at-launch"></a>Wydane podczas uruchamiania
   ...
   |****|**Microsoft Teams Rooms Standardowe urządzenie w wysokości 15 USD miesięcznie**|**Microsoft Teams Rooms Premium 50 USD miesięcznie**|
   |-------------------|:------|:------|
   |Skype dla firm|Yes| |
   |Microsoft Teams|Yes|Yes|
   |system telefoniczny|Yes|Yes|
   |Audiokonferencje|Yes|Yes|
   |Microsoft Intune|Yes|Yes|
   | |Microsoft Teams Rooms Standardowa 15 USD za urządzenie miesięcznie|Microsoft Teams Rooms Premium USD 50 USD na urządzenie miesięcznie|
   |Microsoft Teams Rooms Usługi zarządzane| |Yes|
   |Dostępność na całym świecie|Yes|Na wybranych rynkach|
...
Nowa dodatku Advanced Communications dla usługi Microsoft Teams wersji GCC będzie dostępna od 1 grudnia 2020 r.
...
**Jakie są wymagania wstępne?** Każdy Microsoft 365 lub Office 365, który zawiera Microsoft Teams, może mieć ten dodatek.
...
Informacje na temat mapowania lokalnych praw podwójnego użycia usługi Dynamics 365 dla ofert usługi Project Operations można znaleźć w poniższej tabeli:
   ...
   |-------------------|:------|
   |Oferta operacji D365|Lokalne mapowanie praw podwójnego użycia usługi D365|
   |Operacje Project D365|D365 for Operations, on-prem (AX Server) using the Dyn365 Project Operations SKU (109108477)|
   |Dołączanie operacji Project D365|D365 for Operations, on-prem (AX Server) using the Dyn365 Project Operations SKU (109108477)|
   ...
   |Dołączanie danych finansowych do Project D365|D365 for Operations, on-prem (AX Server) using the Dyn365 Project Operations SKU (109108477)|
   |Ujednolicone operacje D365 — działanie z Project operacji|D365 for Operations, on-prem (AX Server) using the Dyn365 Project Operations SKU (109108477)|
...
Microsoft 365 Business Voice to oparty na chmurze system telefonów dla małych i średnich firm, który jest wbudowany w Office 365. Dodanie usługi Business Voice do subskrypcji usługi Office 365 klienta zapewnia wszystko w jednym rozwiązaniu do komunikacji i współpracy z połączeniami, czatami i spotkaniami w jednej aplikacji, Microsoft Teams.
...
Od 1 listopada 2020 r. do 30 kwietnia 2021 r. nowe i odnowienia subskrypcji usługi Microsoft 365 Business Voice z planem połączeń i usługami Microsoft 365 Business Voice bez planu połączeń podlegają cennikowi rabatu. Microsoft 365 Business Voice z planem połączeń jest objęte rabatem 25% przez 12 miesięcy, a Microsoft 365 Business Voice bez planu wywołań jest objęte rabatem 33% przez 12 miesięcy. 
