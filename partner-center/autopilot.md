...
- Pomiń ustawienia prywatności. To opcjonalne ustawienie profilu rozwiązania Autopilot umożliwia organizacjom, aby nie pytały o ustawienia prywatności podczas procesu OOBE.
...
- Pomiń umowę licencyjną użytkownika oprogramowania (EULA). Począwszy od Windows 10 wersji 1709, organizacje mogą zdecydować się pominąć stronę eula przedstawioną podczas procesu OOBE. Aby Windows ominięcia strony eula rozwiązania Autopilot podczas konfigurowania aplikacji, zobacz Windows rozwiązania [Autopilot eula](#windows-autopilot-eula-dismissal) poniżej.
...
4. Wprowadź nazwę i opis profilu, a następnie skonfiguruj ustawienia OOBE. Wybierz spośród opcji:  
   ...
   - Automatyczne pomijanie stron w konfiguracji<br>
        (Obejmuje *opcje Automatycznie wybieraj konfigurację* dla służbowych oraz *Pomijanie Cortana, OneDrive i rejestracji OEM*)
  
   - Pomijanie umowy licencyjnej użytkownika oprogramowania (EULA)<br> 
       >[!IMPORTANT] 
       >Aby Windows ominięcia strony eula rozwiązania Autopilot podczas konfigurowania aplikacji, zobacz Windows rozwiązania [Autopilot eula](#windows-autopilot-eula-dismissal) poniżej.
...
Korzystając z tej funkcji, zgadzasz się, że pomijanie lub ukrywanie ekranów, które mają na celu powiadomienie lub akceptację warunków, oznacza, że klient uzyskał wystarczającą zgodę i autoryzację, aby ukryć warunki, oraz że w imieniu klienta (w zależności od tego, czy jest to organizacja, czy indywidualny użytkownik),  wyrażać zgody na wszelkie powiadomienia i akceptować wszelkie postanowienia, które mają zastosowanie do klienta. Obejmuje to umowę z warunkami i postanowieniami licencji lub powiadomienie, które byłyby prezentowane użytkownikowi, jeśli użytkownik nie pominął ani nie ukrywał jej za pomocą tego narzędzia. Klient nie może korzystać z tego Windows na tych urządzeniach, jeśli klient nie uzyskał w prawidłowy sposób licencji na oprogramowanie od firmy Microsoft lub licencjonowanych dystrybutorów.