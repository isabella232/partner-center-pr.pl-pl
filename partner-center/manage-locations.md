...
description: Dowiedz się, jak dodać nową lokalizację i jak identyfikator MPN lokalizacji jest używany w programach zachęt, firmie CSP, subskrypcjach i innych transakcjach.
author: vinayks
ms.author: vinayks
...
Firma Contoso ma swoje konto globalne partnera (PGA) w Wielkiej Brytanii. PgA to zarejestrowana firma prawna, a jej globalny identyfikator MPN jest używany do zarządzania całą działalnością nie transakcyjną. Firma Contoso ma również konta lokalizacji partnera (PLA) równoważne podmiotom zależnym lub oddziałom w innej lokalizacji w Zjednoczonym Królestwie, Francji i Stanach Zjednoczonych. W strukturze konta MPN te konta PLA są reprezentowane jako unikatowe identyfikatory MPN lokalizacji. Firmy PLA są używane na potrzeby działalności transakcyjnej, takiej jak program CSP lub zachęty. Wypłaty są powiązane z określonymi lokalizacjami.
...
Aby dodać nowe konto biznesowe programu CSP, rozpocznij od upewniania się, że zostały spełnione wymagania wstępne.
...
1. Zaloguj się do pulpitu [Partner Center nawigacyjnego](https://partner.microsoft.com/dashboard) przy użyciu poświadczeń konta MPN. (Poświadczenia MPN mogą różnić się od poświadczeń CSP).

2. Wybierz ikonę Ustawienia koła zębatego, a następnie wybierz pozycję Konto **Ustawienia**, a następnie pozycję **Prawne.**

3. Na karcie **Partner** sprawdź, czy nie ma komunikatu o błędzie baneru z prośbą o naprawienie lokalizacji zmigrowanych z centrum PMC.  Jeśli lokalizacje nie zostały poprawnie skonfigurowane w centrum PMC i nie zostały jeszcze przejść na komputer, należy zaktualizować te lokalizacje.
...
- **Miasto & pola stan/województwo:** upewnij się, że kombinacja miasta i województwa jest poprawna. Istnieją kraje, w których będzie stosowane menu rozwijane służące do wybierania województwa, a w innych krajach to pole będzie musiało zostać wstawione ręcznie.
...
- **Telefon numer:** upewnij się, że Telefon nie zawiera znaków specjalnych, spacji ani kodu kraju. Wartość wprowadzona w polu Telefon Number będzie zawsze zawierać maksymalnie 10 znaków.

5. Jeśli nie ma komunikatu o błędzie, z usługi **Ustawienia** wybierz pozycję **Konto** Ustawienia, **Profil organizacji,** **Identyfikatory**.
...
2. Wybierz ikonę Ustawienia koła zębatego, pozycję **Konto Ustawienia**, a następnie wybierz pozycję **Profil organizacji.**
...
5. Wybierz **pozycję Dodaj lokalizację**. Spowoduje to utworzenie nowego identyfikatora MPN dla nowej lokalizacji, który będzie można wykorzystać w przypadku transakcji i zachęt CSP.

:::image type="content" source="images/legal-biz.png" alt-text="Dodaj nową firmę z branży prawnej.":::

> [!NOTE]
> Po dodaniu lokalizacji w Partner Center nie można jej usunąć. Jeśli podczas logowania używasz poprawnego identyfikatora **MPN,** w menu po lewej stronie Partner Center po lewej stronie.
...
3. Aby dodać lokalizację, kliknij pozycję Dodaj lokalizację i w wysuwanych menu podaj wymagane szczegóły, takie jak nazwa firmy, adres i podstawowy kontakt dla lokalizacji, którą chcesz dodać do firmy. 
