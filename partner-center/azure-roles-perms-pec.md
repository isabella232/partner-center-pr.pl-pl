...
Azure Event Hubs danych | Umożliwia odbieranie dostępu do Azure Event Hubs zasobów. | Tak | 
...
Właściciel Service Bus azure | Umożliwia pełny dostęp do zasobów usługi Azure Service Bus zasobów. | Tak | 
...
Współautor klasycznego Storage konta | Umożliwia zarządzanie klasycznymi kontami magazynu, ale nie umożliwia dostępu do nich. | Tak | 
Rola usługi Storage operator klucza konta klasycznego | Operatorzy Storage klasycznego konta mogą generować i ponownie generować klucze na klasycznych kontach Storage kont | Tak | 
...
Cost Management czytelnika | Może wyświetlać dane kosztów i konfigurację (np. budżety, eksporty) | Nie | 
urządzenie Data Box współautora | Umożliwia zarządzanie wszystkim w usłudze urządzenie Data Box z wyjątkiem dawania dostępu innym osobom. | Tak | 
urządzenie Data Box czytelnika | Umożliwia zarządzanie usługą urządzenie Data Box z wyjątkiem tworzenia zamówienia lub edytowania szczegółów zamówienia i udzielanie dostępu innym osobom. | Nie | 
...
New Relic współautora konta APM | Umożliwia zarządzanie kontami New Relic Application Performance Management i aplikacjami, ale nie ma do nich dostępu. | Tak | 
...
Administrator zabezpieczeń | Tylko Security Center zabezpieczeń: może wyświetlać zasady zabezpieczeń, wyświetlać stany zabezpieczeń, edytować zasady zabezpieczeń, wyświetlać alerty i zalecenia, odrzucać alerty i zalecenia | Tak | 
...
Czytelnik usługi Site Recovery | Umożliwia wyświetlanie Site Recovery, ale nie wykonywanie innych operacji zarządzania | Nie | 
...
SQL Współautor bazy danych | Umożliwia zarządzanie bazami SQL, ale nie dostęp do nich. Ponadto nie można zarządzać zasadami zabezpieczeń ani ich nadrzędnymi serwerami SQL zabezpieczeń. | Tak | 
SQL Współautor wystąpienia zarządzanego | Umożliwia zarządzanie SQL zarządzanymi i wymaganą konfiguracją sieci, ale nie może udzielać dostępu innym osobom. | Tak | 
...
Współautor danych obiektu blob usługi Storage | Odczyt, zapis i usuwanie usługi Azure Storage i obiektów blob. Aby dowiedzieć się, które akcje są wymagane dla danej operacji danych, zobacz Uprawnienia do wywoływania operacji na danych obiektów blob i kolejek. | Tak | 
...
Logowanie administratora maszyny wirtualnej | Wyświetlanie Virtual Machines portalu i logowanie się jako administrator | Tak | 
...
Logowanie użytkownika maszyny wirtualnej | Wyświetl Virtual Machines portalu i zaloguj się jako zwykły użytkownik. | Tak | 
