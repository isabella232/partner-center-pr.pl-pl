...
description: Utwórz prywatne konta i Azure Marketplace (wersja zapoznawcza) w oknie Azure Portal nowym widokiem Kolekcje. Prywatne Azure Marketplace (wersja zapoznawcza) umożliwiają administratorom określenie rozwiązań innych firm, których mogą używać ich użytkownicy.
...
> [!NOTE]
> W tym artykule omykamy kolekcje Azure Marketplace prywatne. Widok w starszej wersji został zamiast tego objęty [oryginalnym](create-manage-private-azure-marketplace.md) artykułem.
...
> [!NOTE]
> - Prywatne Azure Marketplace na poziomie dzierżawy; Po włączeniu zasady zostaną ustawione dla wszystkich użytkowników w ramach dzierżawy.
...
4. Wybierz **Wprowadzenie,** aby utworzyć Azure Marketplace prywatne (należy to zrobić tylko raz).
    ...
    :::image type="content" source="media/private-azure-new/private-marketplace-get-started.png" alt-text="Pokazuje, jak wybrać pozycję Wprowadzenie w Azure Portal głównym.":::
...
5. Po zakończeniu będziesz mieć wyłączną prywatną Azure Marketplace jedną **kolekcję domyślną**.
    ...
    > [!NOTE]
    > - **Kolekcja domyślna** to zestaw kolekcji generowany przez system z zakresem wszystkich subskrypcji w tej samej dzierżawie.
...
4. Domyślnie podczas dodawania nowej oferty wszystkie bieżące plany zostaną dodane do listy zatwierdzonych. Aby zmodyfikować wybór planu przed dodaniem wybranych elementów, wybierz menu rozwijane na kafelku oferty i zaktualizuj wymagane plany.
...
Edytowanie planów elementu na stronie kolekcji.
...
1. Na **stronie Zarządzanie kolekcją** zostanie wyświetlany baner przedstawiający bieżący stan kolekcji:
...
1. Na stronie **Zarządzanie witryną Marketplace** wybierz kolekcję i użyj górnego paska akcji, aby włączyć lub wyłączyć kolekcję.
   ...
   :::image type="content" source="media/private-azure-new/action-bar.png" alt-text="Pokazuje pasek akcji Zarządzaj witryną Marketplace z przyciskami włączania i wyłączania kolekcji.":::
...
Na stronie Zarządzanie platformą Marketplace jest wyświetlany jeden z tych banerów przedstawiający bieżący stan prywatnych Azure Marketplace:
   ...
   :::image type="content" source="media/private-azure-new/state-enable.png" alt-text="Wyświetla transparent Azure Marketplace włączono.":::
...
2. Nadaj nazwę kolekcji.

3. Wybierz pozycję subskrypcje z menu rozwijanego.
...
4. Wybierz **pozycję Zastosuj** (nie pokazano).
...
- Gdy użytkownik wybierze niezatwierdzoną subskrypcję lub plan, na banerze zostanie  zaaprobowany plan, a przycisk Utwórz zostanie wyłączony. Użytkownik może nadal poprosić o dodanie planu do listy zatwierdzonych (zobacz następną sekcję).
...
1. Za pośrednictwem witryny EA Portal Azure Portal wyłącz usługi innych firm lub ogranicz je do opcji "Tylko bezpłatne jednostki SKU lub BYOL".
